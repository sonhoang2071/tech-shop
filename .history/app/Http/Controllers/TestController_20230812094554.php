...
class TestController extends Controller
{
    ...
    public function gdata2() {
        return $data = DB::select('SELECT MONTH(created_at) AS mm, SUM(tongTien) AS order_total FROM orders WHERE INNER JOIN orderdetail ON order GROUP BY MONTH(created_at)');
    }
