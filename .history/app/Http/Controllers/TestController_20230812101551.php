...
class TestController extends Controller
{
    ...
    public function gdata2() {
        return $data = DB::select('SELECT MONTH(orders.created_at) AS mm, SUM(orders.tongTien) AS order_retail_price, SUM(products.giaMua * orderdetail.soLuong) AS order_cost_price
                        FROM orders
    ...
    public function gdata3(){
        return $data = DB::select('SELECT MONTH(orders.created_at) AS mm, SUM(orders.tongTien)  - SUM(products.giaMua * orderdetail.soLuong) AS order_cost_price
                        FROM orders
                        INNER JOIN orderdetail ON orders.id = orderdetail.id_order
                        INNER JOIN products ON orderdetail.id_sp = products.id
                        GROUP BY MONTH(orders.created_at)');
