...
class TestController extends Controller
{
    ...
    public function gdata2() {
        return $data = DB::select('SELECT MONTH(orders.created_at) AS mm, SUM(orders.tongTien) AS order_total, SUM(products.giaBan * order) AS giaBan
                        FROM orders
                        INNER JOIN orderdetail ON orders.id = orderdetail.id_order
                        INNER JOIN products ON .id_sp = products.id
